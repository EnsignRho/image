...
//////
	...
	SDatum* iDatum_allocate(cvp data, s32 dataLength)
	{
		SDatum* datumNew;
		...
		if (datumNew)
		{
			...
			if (data)
			{
				...
				datumNew->data = (s8*)malloc(dataLength + 1);
				if (datumNew->data)
				{
					memset(datumNew->data, 0, dataLength);
	...
	SDatum* iDatum_make(SDatum* datum, cvp data, s32 dataLength)
	{
		// Make sure our environment is sane
		if (datum)
		{
			datum->data		= (s8*)data;
			datum->length	= ((dataLength >= 0) ? dataLength : (s32)strlen((s8*)data));
		}
	...
	void iDatum_duplicate(SDatum** datumRoot, cvp data, s32 dataLength)
	{
		if (datumRoot)
			*datumRoot = iDatum_allocate(data, dataLength);
	}
...
#ifndef isNumeric
	#define isNumeric(c) (c == '+' || c == '-' || (c >= '0' && c <= '9'))
#endif
...
copy_character:
	...
	{
		return (iDatum_compare(datumLeft, datumRight) == 0);
	}
	...
	s32 iDatum_compare(SDatum* datumLeft, s8* data, s32 dataLength)
	{
		...
		// Make sure our environment is sane
		if (datumLeft && datumLeft->_data && datumLeft->length != 0 && data && dataLength != 0)
		{
