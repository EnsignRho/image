		...
		if (builder && tnAllocationBlockSize != 0)
		{
			...
			if (buffNew)
			{
				...
				*builder = buffNew;
				...
				tnAllocationBlockSize		= max((u32)4096, tnAllocationBlockSize);
...
//
//////
	u32 iBuilder_asciiWriteOutFile(SBuilder* builder, cs8* tcPathname, bool tlAppend)
	{
		return iBuilder_asciiWriteOutFile(builder, (cu8*)tcPathname, tlAppend);
	}
...
//////
	...
	{
		return iBuilder_asciiReadFromFile(builder, (cu8*)tcPathname);
	}
